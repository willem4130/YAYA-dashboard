...
async function analyzeMessage(message: string, context: WorkflowContext) {
  ...
  const intents = {
    ...
    feedback: /(?:improve|better|fix|wrong|not good|change this)/i.test(
      message
    ),
  ...
  return {
    intent:
      Object.entries(intents).find(([_, matches]) => matches)?.[0] || 'general',
    parameters,
    confidence,
      ...
      return {
        ...
        suggestedActions:
          statusInfo.status === 'idle'
            ? ['Run workflow']
            : ['View outputs', 'Check logs'],
      }

    case 'question':
      return await generateInformationalResponse(
...
async function generateInformationalResponse(
  ...
  const workflowGuides = {
    'social-content-gen': {
      purpose:
        'Generate social media content that aligns with YAYA brand aesthetics',
...
function convertToWorkflowParams(
  parameters: string[],
  context: WorkflowContext
): Record<string, any> {
...
async function triggerWorkflowFromChat(
  workflowId: string,
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/workflows/${workflowId}/execute`,
    {
      method: 'POST',
      headers: {
