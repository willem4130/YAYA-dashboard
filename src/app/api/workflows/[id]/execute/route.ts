...
export async function POST(
  ...
  try {
    ...
    const validationResult = validateWorkflowInput(workflowId, body.inputs || {})
    ...
    if (body.userContext?.role && workflowConfig.yayaMetadata.userRoles.length > 0) {
      if (!workflowConfig.yayaMetadata.userRoles.includes(body.userContext.role)) {
        return NextResponse.json(
          { 
    ...
    const webhookPayload = {
      ...
      // Execution context
      context: {
        userId: body.userContext?.userId || request.headers.get('user-id') || 'anonymous',
